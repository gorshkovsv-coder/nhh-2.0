...
<div class="text-center px-4">
  ...
  <div class="flex flex-col items-center gap-2 mt-1">
    <!-- Логотип команды на карточке -->
    <img
      v-if="displayTeamLogo"
      :src="displayTeamLogo"
      :alt="displayTeamName"
      class="w-10 h-10 rounded-full border border-slate-400/70 bg-white object-contain shadow-sm"
    />
  ...
  <div>
    <div class="flex items-center gap-2 mb-1">
      ...
      <!-- Логотип команды в блоке результата -->
      <img
        v-if="lastResult.team.logo_url"
        :src="lastResult.team.logo_url"
        :alt="lastResult.team.name"
        class="w-7 h-7 rounded-full border border-emerald-300 bg-white object-contain"
      />
    </div>
...
const spinningTeamLogo = ref('')      // <— добавили
...
// Логотип для карточки: сначала берём тот, что "накрутили" в спине,
// если пусто — пробуем взять из последнего результата (lastResult)
const displayTeamLogo = computed(() => {
  if (spinningTeamLogo.value) return spinningTeamLogo.value
  return lastResult.value?.team?.logo_url ?? ''
})
...
setTimeout(() => {
  ...
  spinningTeamLogo.value = currentAssignment.team.logo_url || ''
