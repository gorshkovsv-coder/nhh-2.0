...
const lastMatchDate = computed(() => {
  ...
  return formatDate(stats.value.last_match_at)
})
    ...
    <main class="py-6">
      <div class="max-w-5xl mx-auto sm:px-6 lg:px-8 space-y-6">
		...
		<section class="bg-white shadow-sm sm:rounded-lg p-6 space-y-4">
          ...
          <!-- Если это свой профиль — показываем все формы редактирования -->
  ...
  <template v-else>
    <div class="mt-2 flex flex-col sm:flex-row gap-4 items-start">
      ...
      <div class="space-y-1 text-sm">
        ...
        <div v-if="user.email" class="text-xs text-gray-600">
          Email:
          <span class="font-mono">{{ user.email }}</span>
        </div>

        <div v-if="user.psn" class="text-xs text-gray-600">
          PSN:
          <span class="font-mono">{{ user.psn }}</span>
        </div>
      </div>
    </div>
  </template>
        ...
        <section class="bg-white shadow-sm sm:rounded-lg p-6">
          ...
          <div v-if="lastMatches.length" class="space-y-3">
            <div
              ...
              :key="m.id"
              class="border rounded-lg p-3 sm:p-4 flex flex-col gap-3 sm:grid sm:grid-cols-[minmax(0,220px)_minmax(0,420px)_auto] sm:items-center sm:gap-6"
            >
              <div class="min-w-0">
              ...
              <div class="min-w-0 sm:max-w-[420px]">
                ...
                <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-4">
                  <div class="flex items-center gap-2 min-w-0">
                    ...
                    <div class="min-w-0">
                      ...
                      <p
                        class="text-[11px] break-words leading-tight"
                  ...
                  <div class="text-sm font-semibold text-gray-900 text-center sm:px-1">
                    {{ scoreLabel(m) }}
                  </div>
              ...
              <div class="flex flex-col gap-2 sm:items-end sm:gap-1">
                <p v-if="m.status_label" class="text-xs text-gray-500">
                  {{ m.status_label }}
                </p>
                <Link
                  :href="`/matches/${m.id}`"
                  class="inline-flex items-center justify-center px-3 py-2 rounded-lg text-sm font-medium bg-slate-900 text-white hover:bg-slate-800 w-full md:w-auto"
                >
                  Открыть матч
                </Link>
              </div>
          ...
          >
            У игрока пока нет сыгранных матчей. Когда появятся результаты, последние игры будут здесь.
          </div>
        </section>
        ...
        <section
          v-if="isOwner"
          class="bg-white shadow-sm sm:rounded-lg p-6 space-y-4 border border-red-100"
        >
